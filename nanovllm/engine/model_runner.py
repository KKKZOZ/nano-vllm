...
class ModelRunner:
    ...
    def warmup_model(self):
        ...
        logger.info(
            f"Warming up model with {num_seqs} sequences of length {max_model_len}"
        )
    ...
    def prepare_block_tables(self, seqs: list[Sequence], max_blocks: int | None = None):
        ...
        block_tables = [
            blocks + [-1] * (max_len - len(blocks)) for blocks in block_tables
        ]
    ...
    def prepare_prefill(self, seqs: list[Sequence]):
        ...
        # cu_seqlens_q = [0, 10, 25, 40] 表示有 3 个序列，长度分别为 10、15、15
        cu_seqlens_q = [0]
        cu_seqlens_k = [0]
        max_seqlen_q = 0
        max_seqlen_k = 0
    ...
    def capture_cudagraph(self):
        ...
        if config.decode_window_blocks is not None:
            max_num_blocks = min(max_num_blocks, config.decode_window_blocks)
